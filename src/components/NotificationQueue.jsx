...
export default function NotificationQueue({ user }) {
  const [form, setForm] = useState({ title: '', message: '', phone_number: '', target_role: 'parent', scheduled_for: nowLocal(), max_attempts: 3 })
  ...
  async function submit(event) {
    ...
    try {
      ...
      setMessage(result.message); setForm(current => ({ ...current, title: '', message: '', phone_number: '', scheduled_for: nowLocal() })); await load()
